...
class Qtelecom
{
    ...
    private $user = null; // ваш логин в системе
    private $pass = null; // ваш пароль в системе
    private $hostname = 'service.qtelecom.ru'; // host замените на адрес сервера указанный в меню "Поддержка -> протокол HTTP"  без префикса http://
    private $path = '/public/http/';
    private $on_ssl = 0; // 1 - использовать HTTPS соединение, 0 - HTTP
    ...
    private function get_post_request($inprivates)
    {
        $inprivates['user'] = ($this->user);
        $inprivates['pass'] = ($this->pass);
        $inprivates['CLIENTADR'] = isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : false;
        $inprivates['HTTP_ACCEPT_LANGUAGE'] = isset($_SERVER['HTTP_ACCEPT_LANGUAGE']) ? $_SERVER['HTTP_ACCEPT_LANGUAGE'] : false;
        $PostData = http_build_query($inprivates, '', '&');
        $len = strlen($PostData);
        $nn = "\r\n";
        ...
        if (($fp = @fsockopen(
            ...
            throw new QtelecomException('Невозможно соединиться с сервером.');
        }
    }
    ...
    {
        return $this->post_data['data'][] = $inv;
    }
    ...
    public function post_mms_message($subj, $mes, $f_data, $target)
    {
        if (is_array($target)) {
            $target = implode(',', $target);
        }
    ...
    private function get_multipost_request($inprivates, $file_data)
    {
        define("CRLF", "\r\n");
        ...
        $inprivates['CLIENTADR'] = isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : false;
        $inprivates['HTTP_ACCEPT_LANGUAGE'] = isset($_SERVER['HTTP_ACCEPT_LANGUAGE']) ? $_SERVER['HTTP_ACCEPT_LANGUAGE'] : false;
        foreach ($inprivates as $field => $value) {
        ...
        if (($fp = @fsockopen(
            ...
            if (!$fp2) {
                throw new QtelecomException("Невозможно открыть файл {$file_data['path']}");
            }
    ...
    public function post_mms_message_phl($subj, $mes, $f_data, $phl_codename)
    {
